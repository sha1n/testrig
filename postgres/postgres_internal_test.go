...
func TestPostgres_Unit_Defaults(t *testing.T) {
	...
	props, err := pg.Start(context.Background(), testrig.StubEnvHandle("test", slog.Default(), nil))
...
func TestPostgres_Unit_HostFailure_CleansUp(t *testing.T) {
	...
	_, err := pg.Start(context.Background(), testrig.StubEnvHandle("test", slog.Default(), nil))
	require.Error(t, err)
...
func TestPostgres_Unit_LogStreaming_ForwardsLogs(t *testing.T) {
	...
	_, err := pg.Start(context.Background(), testrig.StubEnvHandle("test", logger, nil))
	require.NoError(t, err)
