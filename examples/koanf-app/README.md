...
Demonstrates integrating `testrig-go` with applications that use [Koanf](https://github.com/knadh/koanf) for configuration management, using parallel-safe in-memory injection.
...
- Bridges from the testkit's property keys (`pg.dsn`) to the application's config keys (`DATABASE_URL`) at the consumption site. Worth noting: koanf treats `.` as a delimiter, so passing `pg.dsn` verbatim would create a nested map rather than a flat key.
