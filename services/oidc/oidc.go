...
// fixture for testrig. It implements api.Service and exposes:
//
...
// Start implements api.Service.
func (i *Issuer) Start(ctx context.Context, env api.EnvHandle) (api.Properties, error) {
	i.mu.Lock()
	defer i.mu.Unlock()
...
// buildProperties returns the api.Properties published by Start.
// Default keys "<name>.<suffix>" unless overridden via With*PropertyName.
func (i *Issuer) buildProperties() api.Properties {
...
// Stop implements api.Service. Idempotent.
