...
`Start` returns a `api.Properties` map (also available via `env.Properties()`) containing the following seven keys. All URL values are empty strings if `Start` has not been called.
...
func TestMyHandler_AcceptsValidToken(t *testing.T) {
    ...
    _, err := iss.Start(context.Background(), api.StubEnvHandle("test", slog.Default(), nil))
    if err != nil {
        t.Fatal(err)
    }
    t.Cleanup(func() { _ = iss.Stop(context.Background()) })
