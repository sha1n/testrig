...
// The exported WireMock type implements api.Service.
package wiremock
...
// api.Service so it can be added to a testrig.Env, and exposes typed-client
...
// Start implements api.Service. Returns an error if called while a
// previous Start is still active (i.e. Stop has not been called).
func (t *WireMock) Start(ctx context.Context, env api.EnvHandle) (api.Properties, error) {
	if t.container != nil {
...
// Stop implements api.Service. Safe to call before Start or twice in
