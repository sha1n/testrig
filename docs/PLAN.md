...
- [x] **refactor: introduce `EnvHandle` on `Service.Start`** — replaced the `*slog.Logger` parameter on `Service.Start` with an `EnvHandle` interface (`Name()`, `Logger()`, `Properties()`), giving services read access to env-scoped state. Services in later stages can now read properties published by services in earlier stages of the same track via `env.Properties()`, so derived properties are computed by a regular `Service` rather than needing a separate provider concept. Added public `testrig.StubEnvHandle(name, logger, props)` helper for standalone tests of `Service` implementations. Breaking change to the `Service` interface; postgres / oidc / wiremock services and the example `SchemaSeed` migrated. `LifecycleHook` is unchanged.
